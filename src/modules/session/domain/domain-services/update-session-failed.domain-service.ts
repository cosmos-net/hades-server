...
import { ISessionUpdateFailedPrimitives } from '@session/domain/schemas/session.schema-primitives';

export class UpdateSessionFailedDomainService {
  constructor(private readonly repositorySession: ISessionRepositoryContract) {}
  async go(schema: ISessionUpdateFailedPrimitives): Promise<SessionModel> {
    ...
    sessionModel.changeStatus(SessionStatusEnum.ACTIVE);
