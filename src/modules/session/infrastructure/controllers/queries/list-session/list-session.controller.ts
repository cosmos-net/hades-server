...
import { ListSessionFilter } from '@session/infrastructure/controllers/queries/list-session/list-session.filter-dto';
...
export class ListSessionController {
  ...
  async list(@Payload() listSessionDto: ListSessionInputDto): Promise<ListSessionOutputDto> {
    try {
      ...
      const filtersMap = ListSessionFilter.toFilterMap(listSessionDto);
